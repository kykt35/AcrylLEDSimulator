...
- グレースケールまたは階調の彫刻用PNGを保存したい制作担当者
...
- 画像が未選択の場合、ダウンロードボタンは無効になる。
- ダウンロード処理中はボタンを無効にする。
- Blob生成後に編集スナップショットを保存し、ブラウザのローカルダウンロードを開始する。
- 成功時は結果画面へ遷移せず、`role="status"` の成功toastを表示する。
- toastには出力形式と継続編集できることを表示し、閉じた後も画像と設定を維持する。
...
- シミュレーション画像のBlob生成後、ローカルダウンロードを開始する直前にエディタスナップショットを保存する。
...
- 読み込み時の彫刻調整値 `toneMode` が不足または不正な場合は `grayscale` に補完する。
- 読み込み時の彫刻調整値 `toneLevels` は2から8、`edgeWidth` は1から5の範囲へ正規化する。
...
ダウンロード失敗時は `ErrorNotice` を表示し、現在の画像と設定を保持する。サーバー保存APIへの再試行や結果画面への遷移は行わない。
...
- サーバー永続化、共有URL、保存履歴、結果専用画面の要否は未確認であり、現行MVPには含めない。
