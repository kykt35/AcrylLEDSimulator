...
- MVPでは両URLに機能差を設けず、画像入力からダウンロードまで同じ1画面で完結する。
- ダウンロード成功時もURLを変更せず、結果専用画面へ遷移しない。
...
- ダウンロード成功時は成功toastを表示し、同じ画像と設定で編集を継続できる。
...
- 公開時にSEO上のcanonical URLや `/simulator` のリダイレクトを設定するかは未決定。MVPの画面仕様では両URLを同等に扱う。
